...
rule add_group:
    ...
    shell:
       """samtools addreplacerg --threads 64 -r "@RG\\tID:{params.name}\\tSM:{params.name}\\tPL:ILLUMINA" {params.input} -o {output.bam_rg}"""
...
rule collect_map_stats:
    ...
    shell:
        "wes/modules/scripts/align_getMapStats.py -f {params.files} > {output} 2>>{log}"
    #run:
    #    files = " -f ".join(input)
