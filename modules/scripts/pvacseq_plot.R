...
# ### testing
# input = "~/Documents/rnaseq/data/Merged.filtered.condensed.ranked.addSample.tsv"
# outdir <- "~/Documents/rnaseq/data/"

##############################################################################
### Reading matrix
input_table = read.table(input, header=TRUE,sep = '\t')
input_table$is_novel = ifelse(input_table$Fold.Change>1 & input_table$MT.IC50 <500, "novel","no-novel")
...
png(file = paste(outdir, "Patient_count_epitopes_plot.png",sep = ""), res = 300, width = 1000, height = 1000)
...
ggplot(input_table, aes(y = MT.IC50, x = WT.IC50,colour = is_novel)) +
  geom_point()+
...
###merge g1 and g2
png(file = paste(outdir, "HLA_epitopes_fraction_plot.png",sep = ""), res = 300, width = 1500, height = 2000)
